    ...
    public void onClick(View v) {
        ...
        if (v.getId() == BUT_CREATE) {
            if (todo != null) {
                ...
                todo.status = status;
                //TodoRepository.getInstance().removeTodo(todo);
                TodoRepository.getInstance().addTodo(todo);
            } else {
                TodoRepository.getInstance().addTodo(new Todo(0 ,
                        headText.getText().toString(),
